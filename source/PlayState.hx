...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch(SONG.stage)
		{
		    ...
		    case 'basement':
			{
			    ...
			    var basement:FlxSprite = new FlxSprite(-520, -420).loadGraphic(Paths.image('basement'));
			...
			/*	staticlol = new FlxSprite();
				staticlol.frames = Paths.getSparrowAtlas('staticlol', 'shared');
				staticlol.animation.addByPrefix('idle', 'Static', 24, false);
				staticlol.setGraphicSize(Std.int(staticlol.width * 6));
				staticlol.animation.play('idle');
				staticlol.antialiasing = true;
				staticlol.screenCenter();
			//	staticlol.visible = false;
						add(staticlol); */
						
			    var gapers:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.image('gapers'));
			    gapers.setGraphicSize(Std.int(gapers.width * 0.8));
			    gapers.updateHitbox();
		    ...
		    case 'cathedral':
			{
			    ...
			    var cathedral:FlxSprite = new FlxSprite(-520, -230).loadGraphic(Paths.image('cathedral'));
			    cathedral.setGraphicSize(Std.int(cathedral.width * 0.85));
				...
				holylight = new FlxSprite(-200, -100);
				holylight.frames = Paths.getSparrowAtlas('holylight');
				holylight.animation.addByPrefix('idle', 'holylight', 24, false);
				holylight.setGraphicSize(Std.int(holylight.width * 1.2));
				holylight.animation.play('idle');
		...
		unknown = new FlxSprite(860, 690).loadGraphic(Paths.image('unknown', 'shared'));
		unknown.setGraphicSize(Std.int(unknown.width * 0.5));
		unknown.updateHitbox();
		unknown.antialiasing = true;
		unknown.scrollFactor.set();
		...
		{	
			if (SONG.stage == 'basement')
				{
					staticlol = new FlxSprite();
					staticlol.frames = Paths.getSparrowAtlas('staticlol', 'shared');
					staticlol.animation.addByPrefix('idle', 'Static', 24, false);
					staticlol.setGraphicSize(Std.int(staticlol.width * 8));
					staticlol.antialiasing = true;
					staticlol.screenCenter();
					staticlol.visible = false;
							add(staticlol);
	...
	*/
	...
	function fly()
		{
			FlxTween.tween(dad, {y: -700}, 0.3, {ease: FlxEase.expoInOut});
		}
	...
	/*	if (dad.animation.startsWith('singUP-alt')){
			fly();
		} */
		...
		#end
		if (SONG.song.toLowerCase() == 'innocence-glitched')
		{

		    if (curStep == 1088)
			{
			    remove(dad);
			    dad = new Character(100, 430, 'isaacshifting');
			    add(dad);
			...
			{
			    remove(dad);
			    dad = new Character(100, 430, 'funkyisaac');
			    add(dad);
			    staticlol.visible = false;
			}
		}
